		...
		function getPhotos() {
			...
			$http.get($scope.requestURL).then(function (res){
				...
				$.each($scope.photos, function (index, value) {
					...
					html_slide += '<li class="pane3 id="' + value.username + '"><div ng-if="false" class="img" pid="' + value.username + '" style="background: url(\''+ root + '/res/photos/'+value.image_path +'\') no-repeat scroll center center;background-size: cover;"></div>';

					html_slide += "<div style='height:22px;'></div>";

					html_slide += '<div ng-if="false" style="padding-top:0px;"><!--i onclick="goScrap(' + value.username + ');	 $(this).addClass(\'md-red\');" class="material-icons md-light md-inactive star-btn">&#xE838;</i--><p style="font-size:12px;">' + (value.explanation ? value.explanation : "나의 데일리룩") + '</p></div><div class="like"></div><div class="dislike"></div></li>';
				});
